...
@app.route('/catalog/<category_name>/<item_title>.json',
           methods=['GET', 'POST'])
def ItemDescriptionJSON(category_name, item_title):
    engine = create_engine('sqlite:///sportscatalog.db')
    Base.metadata.bind = engine
    DBSession = sessionmaker(bind=engine)
    session = DBSession()
    ...
    return jsonify(Item_Description=item_title.serialize)
    ...
    if 'access_token' in login_session:
        ...
        if 'username' not in login_session or creator.email != \
           login_session['username']:
            flash('Item %s can not be edited \
    ...
    if 'access_token' in login_session:
        ...
        if 'username' not in login_session \
           or creator.email != login_session['username']:
            flash('Item %s can not be deleted by \
            this user' % itemToDelete.title)
            return redirect(url_for('showCategories',
                            categories=categories, loggedin=loggedin))
    else:
        loggedin = False
